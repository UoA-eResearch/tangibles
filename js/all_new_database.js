(function () {
    ...
    var db;                 // javascript web database
    ...
    function preload(str, src){
      var is_exist =-1;
      // using javascript database
      console.log("preload begins...");
      db.transaction(function (tx) {
        tx.executeSql('SELECT * FROM imagelib WHERE NAME = ?', [str], function(tx, results) {
              ...
              if (xhr.status === 200) {
                ...
                fileReader.onload = function (evt) {
                  ...
                  try {
                    ...
                    db.transaction(function (tx) {
                      tx.executeSql('INSERT INTO imagelib (name, content) VALUES (?,?)', [str, result]);
                    });
                  }
                  catch (e) {
                    console.log("Database storage failed: " + e);
                  }
                };
    ...
    function createTables(db) {
      db.transaction(
        function (transaction) {
          ...
          transaction.executeSql('CREATE TABLE imagelib (name TEXT , content TEXT)');
        }
      );
    ...
    function initialize(){
      ...
      try {
        if (!window.openDatabase) {
          ...
          var version = '1.0';
          var displayName = 'My Image Database';
    ...
    document.getElementById("add").onclick = function(){
      ...
      img.onload = function(e) {
        ...
        binaryReader.onloadend=function(d) {  
          ...
          // save into database
          db.transaction(function (tx) {
            tx.executeSql('INSERT INTO imagelib (name, content) VALUES (?,?)', [str, data]);
          });
      ...
      db.transaction(function(tx){
        tx.executeSql('SELECT * FROM imagelib WHERE NAME = ?', [str], function(tx, results) {
          ...
          if(len == 0) {
            ...
            imageObj.src = results.rows.item(0).content;
          }
        });
      });
